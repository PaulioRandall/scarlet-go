...
func dummyLexemes() (l1, l2, l3, l4 token.Lexeme) {
	l1 = token.MakeTok("true", token.TRUE)
	l2 = token.MakeTok("1", token.NUMBER)
	l3 = token.MakeTok("abc", token.STRING)
	l4 = token.MakeTok("i", token.IDENT)
	return
}
...
func chainLexemes(lexs ...token.Lexeme) (head, tail *node, size int) {
